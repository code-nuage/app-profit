...
return function(app)
    ...
    .route({method = "GET", path = "/user/:get"}, function(req, res)
        if string.match(req.params.get, "^%d+$") then
            res.body, res.code, res.headers["Content-Type"] = controller.read_by_id(tonumber(req.params.get))
        end
        if string.match(req.params.get, "^[%w%.%-_]+@[%w%.%-_]+%.%a%a+$") then
            res.body, res.code, res.headers["Content-Type"] = controller.read_by_email(req.params.get)
        end
    end)
